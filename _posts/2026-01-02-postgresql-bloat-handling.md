...
The pgstattuple extension can inspect physical data layout and report the actual percentage of dead space in a table or index. It works by reading either all or part of the relation files, depending on visibility map coverage.

The pgstattuple extension inspects the physical layout of tables and indexes to provide accurate information about bloat. 

- For **tables**, the approximate variant uses the visibility map to skip pages and reduce I/O.
- For **indexes**, however, it always performs a full scan of the index file.
...
We recommend using this method selectively — for the most critical tables and indexes only — and ideally during non-peak hours. Example scripts are available in the GitHub repository: [table_bloat_approx.sql](https://github.com/dataegret/pg-utils/blob/master/sql/table_bloat_approx.sql) for tables and [index_bloat.sql](https://github.com/dataegret/pg-utils/blob/master/sql/index_bloat.sql) for indexes.
...
> ⚠️ **Warning:** If you run pg_repack remotely and the connection drops (e.g., via SSH or load balancer), it can leave behind long-lived locks and block access to the table — especially with `--no-kill-backend`. See: [pg_repack issue #140](https://github.com/reorg/pg_repack/issues/140)

> ⚠️ **Important:** If a repack operation fails or is interrupted, use the documented cleanup steps to remove temporary objects before retrying. Don't skip cleanup — leftover objects can block future repack attempts.
